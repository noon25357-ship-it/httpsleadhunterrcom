...
function calcScore(lead: Omit<SmartLead, 'opportunityScore' | 'opportunityReasons' | 'pipelineStatus' | 'notes' | 'lastContact' | 'nextFollowUp'>): { score: number; reasons: string[] } {
  ...
  if (['الرياض', 'جدة', 'مكة', 'الدمام', 'المدينة'].includes(lead.city)) { score += 10; reasons.push('في مدينة رئيسية'); }
...
export const MOCK_SMART_LEADS: SmartLead[] = [
  ...
  buildLead({ id: 'sl-5', businessName: 'مركز رؤية البصر', city: 'المدينة', category: 'عيادات', websiteStatus: 'no-website', phone: '0572225566' }),
