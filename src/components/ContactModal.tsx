...
const services = [
  { key: "website", label: "موقع إلكتروني" },
  { key: "marketing", label: "تسويق" },
  { key: "social", label: "سوشال ميديا" },
];
...
const ContactModal = ({ lead, onClose }: ContactModalProps) => {
  ...
  const message = getDefaultMessage(service, tone);
      ...
      >
        ...
        >
          ...
          <div className="px-5 pb-5 space-y-3">
            ...
            <div className="bg-secondary rounded-xl p-4 text-sm leading-relaxed text-secondary-foreground whitespace-pre-line">
              {message}
            </div>
            <button
              onClick={() => copyText(message, "msg")}
              className="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl border border-border text-sm font-medium text-foreground hover:bg-secondary transition-colors"
            >
